...
Date: 04-07-2026
Question: The globally fit XGBoost model (Sprint 3 cached hyperparameters: learning rate 0.01, 50 trees, max depth 4) shows exactly zero SHAP importance on 15 of its 17 features in every one of the six regimes, with only gdp_growth and gdp_lag_4 carrying any SHAP importance (about 82 percent and 18 percent of total SHAP importance respectively; raw mean absolute SHAP 0.144 and 0.031, persisted in results/shap/global_importance.csv). Is this a bug in the SHAP pipeline, under-fitting from overly conservative hyperparameters, or a genuine lack of exploitable signal in the macroeconomic predictors?
Options considered:
...
## Decision: ONS release and measure used to validate the 2026 forecasts
Date: 08-07-2026
Question: Which ONS release does the 2026 Q1 validation figure come from, and is it the same measure the model was trained on?
Decision: Validate against ONS series IHYQ, "Gross Domestic Product: Quarter on Quarter growth: CVM SA %", which is the same real (chained volume measure), seasonally adjusted, quarter-on-quarter measure the model's target was built from. The 2026 Q1 figure of 0.6 percent used in validation comes from the Quarterly National Accounts (QNA) release dated 29 June 2026: in the cached ONS response the 2026 Q1 datapoint has sourceDataset QNA and updateDate 2026-06-29. The first estimate, released about six weeks after the quarter ended (mid-May 2026), also printed 0.6 percent, so the two vintages agree and the validation result does not depend on which is used. Q2 2026 will be validated the same way once ONS publishes it; the next scheduled release is 13 August 2026.
Rationale: The forecast target and the validation figure must be the same measure, or the comparison is meaningless. The training target is ONS IHYQ (CVM SA quarter-on-quarter GDP growth) and the validation figure is the same series and code, so measure consistency holds by construction. ONS publishes GDP more than once per quarter (a first estimate, then the fuller QNA release, then later revisions) and the figures can differ between vintages, so the specific release is recorded here for reproducibility: the 0.6 percent Q1 2026 actual is the QNA vintage dated 29 June 2026, cached locally under data/raw/_api_responses/ons/. Because the first estimate and the QNA release both show 0.6 percent, the Q1 validation error (forecast 0.444 against actual 0.6) is robust to the vintage choice. This entry records the release for the Q1 validation already done and fixes the same protocol for the Q2 validation to follow in August.
